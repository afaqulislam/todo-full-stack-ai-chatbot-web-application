"""MCP tool for completing tasks by description/title - follows constitution requirements."""
import logging
...
async def complete_task_by_description_tool(
    ...
    """
    MCP tool to complete a task by its description/title via the existing Task service layer.
    This allows completing when user provides a task title/description instead of an ID.
    ...
    updated_task = await TaskAdapterService.complete_task(
        session=session,
    ...
    if not updated_task:
        raise ValueError(f"Task with id {best_match['task_id']} could not be completed")

    # Log the successful match for debugging
    logging.info(f"Task matching '{task_description}' was found and completed. Match score: {best_match['score']}")

    # Return structured JSON as required by constitution
    return {
        "task_id": str(updated_task.id),  # Return the UUID for identification
        "status": updated_task.status.value,  # Status indicating the task status
        "title": updated_task.title,  # Return the title of the completed task
        ...
        "completed": updated_task.status.value == "completed"  # Whether task is now completed
    }


def _find_best_task_match(task_description: str, tasks: list) -> Dict[str, Any]:
    """
...
def validate_complete_task_by_description_params(params: Dict[str, Any]) -> Dict[str, Any]:
    """Validate parameters for the complete_task_by_description tool."""
    ...
    # completed parameter is optional, defaults to True
