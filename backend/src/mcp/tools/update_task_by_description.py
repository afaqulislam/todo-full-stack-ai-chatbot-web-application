"""MCP tool for updating tasks by description/title - follows constitution requirements."""
import logging
...
async def update_task_by_description_tool(
    ...
    """
    MCP tool to update a task by its description/title via the existing Task service layer.
    This allows updating when user provides a task title/description instead of an ID.
    ...
    updated_task = await TaskAdapterService.update_task(
        session=session,
    ...
    if not updated_task:
        raise ValueError(f"Task with id {best_match['task_id']} could not be updated")

    # Log the successful match for debugging
    logging.info(f"Task matching '{task_description}' was found and updated. Match score: {best_match['score']}")
    ...
    return {
        ...
        "title": updated_task.title,  # Return the title of the updated task
...
def validate_update_task_by_description_params(params: Dict[str, Any]) -> Dict[str, Any]:
    """Validate parameters for the update_task_by_description tool."""
